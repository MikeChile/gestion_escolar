...
//agregar una nota de conducta al alumno
$('#formularioConducta').on('submit', function (event) {
	event.preventDefault();
	var tipoConducta = $('#tipoConducta').val();
	var descripcionConducta = $('#descripcionConducta').val();
	var idAlumnoConducta = $('#idAlumnoConducta').val();
	console.log(idAlumnoConducta); // Verifica si el valor es correcto

	$.ajax({
		type: 'POST',
		url: '../ajax/conducta.php?op=registrarConducta',
		data: {
			tipoConducta: tipoConducta,
			descripcionConducta: descripcionConducta,
			idAlumnoConducta: idAlumnoConducta
		},
		success: function (data) {
			...
			$.ajax({
				type: 'GET',
				url: '../ajax/conducta.php?op=mostrarConducta&id_alumno=' + idAlumnoConducta,
				dataType: 'json',
				success: function (data) {
					...
					$.each(data, function (index, value) {
						cuerpoTabla.append('<tr>' +
							...
							'<td>' + value.fecha + '</td>' +
							'<td><button class="btn btn-danger btn-xs" onclick="eliminarConducta(' + value.idAlumnoConducta + ')">Eliminar</button></td>' +
							'</tr>');
					});
				}
			});
...
function eliminarConducta(id) {
	if (confirm("¿Estás seguro de que deseas eliminar este registro de conducta?")) {
		$.ajax({
			type: 'POST',
			url: '../ajax/conducta.php?op=eliminarConducta',
			data: { id: id },
			success: function (data) {
				console.log(data);
				// Recargar la tabla con los nuevos datos
				$.ajax({
					type: 'GET',
					url: '../ajax/conducta.php?op=mostrarConducta&id_alumno=' + $('#idAlumnoConducta').val(),
					dataType: 'json',
					success: function (data) {
						var cuerpoTabla = $('#cuerpoTablaConducta');
						cuerpoTabla.empty();
